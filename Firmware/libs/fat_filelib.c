...
static FL_FILE* _open_file(const char *path)
{
    ...
    if (fatfs_get_file_entry(&_fs, file->parentcluster, file->filename,&sfEntry)){ //bateo aqui
...
void* fl_fopen(const char *path, const char *mode)
{
    ...
    if (flags & FILE_READ){
		//printf("reading\n");
        file = _open_file(path);  //Problema aqui
        //~ if(file)
			//~ printf("file ok\n");
	}
...
int fl_fread(void * buffer, int size, int length, void *f )
{
    ...
    //printf("file->bytenum: %d, sector: %d, FAT_SECTOR_SIZE: %d\n",file->bytenum, sector, FAT_SECTOR_SIZE);
...
void fl_listdirectory(const char *path)
{
    ...
    if (fl_opendir(path, &dirstat))
    {
        ...
        while (fl_readdir(&dirstat, &dirent))			//fl_readdir(&dirstat, &dirent) == 0)
        {			
#if FATFS_INC_TIME_DATE_SUPPORT
            int d,m,y,h,mn,s;
